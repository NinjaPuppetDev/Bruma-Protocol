...
function ethWrite(runtime2, evmClient, address, abi, functionName, args, gasLimit) {
  const callData = encodeFunctionData({ abi, functionName, args });
  const reportData = encodeAbiParameters(parseAbiParameters("address target, bytes data"), [address, callData]);
  const report2 = runtime2.report({
    encodedPayload: Buffer.from(hexToBytes2(reportData)).toString("base64"),
    encoderName: "evm",
    signingAlgo: "ecdsa",
...
var onSettlementCron = (runtime2) => {
  const { config } = runtime2;
  runtime2.log("=== Bruma Settlement Workflow triggered ===");
  ...
  const activeOptions = ethCall(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "getActiveOptions");
  runtime2.log(`Active options: ${activeOptions.length}`);
  if (activeOptions.length === 0)
    return "No active options to process.";
  const results = {
    settlementRequested: [],
    settled: [],
    bridged: [],
  ...
  for (const tokenId of activeOptions) {
    try {
      const option = ethCall(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "getOption", [tokenId]);
      ...
      if (status === OptionStatus.Active) {
        ...
        if (!expired) {
          runtime2.log(`TokenId ${tokenId}: not yet expired, skipping.`);
          results.skipped.push(tokenId);
          continue;
        }
        runtime2.log(`TokenId ${tokenId}: expired — requesting settlement...`);
        ethWrite(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "requestSettlement", [tokenId], config.gasLimit);
        results.settlementRequested.push(tokenId);
        continue;
      }
      if (status === OptionStatus.Settling) {
        runtime2.log(`TokenId ${tokenId}: settling...`);
        try {
          ethWrite(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "settle", [tokenId], config.gasLimit);
          results.settled.push(tokenId);
        } catch (e) {
          if (e.message?.includes("OracleNotFulfilled")) {
            runtime2.log(`TokenId ${tokenId}: oracle pending, will retry next run.`);
            results.skipped.push(tokenId);
          } else {
            throw e;
          }
      ...
      if (status === OptionStatus.Settled) {
        const ownerAtSettlement = option.state.ownerAtSettlement;
        const finalPayout = BigInt(option.state.finalPayout);
        if (finalPayout === 0n) {
          runtime2.log(`TokenId ${tokenId}: zero payout (OTM), skipping.`);
          results.skipped.push(tokenId);
          continue;
        }
        const isEscrow = ethCall(runtime2, evmClient, config.brumaFactoryAddress, FACTORY_ABI, "isRegisteredEscrow", [ownerAtSettlement]);
        if (!isEscrow) {
          runtime2.log(`TokenId ${tokenId}: same-chain owner — skipping bridge.`);
          results.skipped.push(tokenId);
          continue;
        }
        const alreadyClaimed = ethCall(runtime2, evmClient, ownerAtSettlement, ESCROW_ABI, "claimed", [tokenId]);
        if (alreadyClaimed) {
          runtime2.log(`TokenId ${tokenId}: already claimed, skipping.`);
          results.skipped.push(tokenId);
          continue;
        }
        runtime2.log(`TokenId ${tokenId}: escrow ${ownerAtSettlement} — ` + `bridging ${finalPayout.toString()} wei via CCIP...`);
        ethWrite(runtime2, evmClient, ownerAtSettlement, ESCROW_ABI, "claimAndBridge", [tokenId], config.gasLimit);
        results.bridged.push(tokenId);
      }
    } catch (err) {
      runtime2.log(`TokenId ${tokenId}: ERROR — ${err.message}`);
...
var fetchGroqRiskSummary = (sendRequester, params) => {
  ...
  const reinEth = (Number(params.reinsuranceCapacity) / 1000000000000000000).toFixed(4);
...
var onRiskCron = (runtime2) => {
  const { config } = runtime2;
  runtime2.log("=== Bruma Vault Risk Guardian triggered ===");
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: config.chainName,
    isTestnet: true
  ...
  if (!network248)
    throw new Error(`Unknown chain: ${config.chainName}`);
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  const metricsArr = ethCall(runtime2, evmClient, config.brumaVaultAddress, VAULT_ABI, "getMetrics");
  const tvl = metricsArr[0];
  const currentUtilBps = Number(metricsArr[3]);
  const netPnL = metricsArr[6];
  runtime2.log(`Vault: TVL=${tvl.toString()} | ` + `Utilization=${currentUtilBps / 100}% | ` + `NetPnL=${netPnL.toString()}`);
  const reinsuranceCapacity = ethCall(runtime2, evmClient, config.reinsurancePoolAddress, REINSURANCE_POOL_ABI, "availableCapacity");
  const maxDrawableNow = ethCall(runtime2, evmClient, config.reinsurancePoolAddress, REINSURANCE_POOL_ABI, "maxDrawableNow");
  runtime2.log(`ReinsurancePool: capacity=${reinsuranceCapacity.toString()} | ` + `maxDrawableNow=${maxDrawableNow.toString()}`);
  const activeOptions = ethCall(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "getActiveOptions");
  if (activeOptions.length === 0) {
    runtime2.log("No active options — vault risk is zero.");
  ...
  for (const tokenId of activeOptions) {
    const option = ethCall(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "getOption", [tokenId]);
    const status = Number(option.state.status);
    if (status === OptionStatus2.Expired) {
      try {
        runtime2.log(`TokenId ${tokenId}: expired — requesting settlement...`);
        ethWrite(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "requestSettlement", [tokenId], config.gasLimit);
        settledCount++;
      } catch (e) {
        runtime2.log(`TokenId ${tokenId}: requestSettlement failed — ${e.message}`);
      }
      continue;
    }
    if (status === OptionStatus2.Settling || status === OptionStatus2.Settled) {
      runtime2.log(`TokenId ${tokenId}: status=${status} — skipping forecast.`);
      continue;
    }
    const lat = option.terms.latitude;
    const lon = option.terms.longitude;
    const locationKey = `${lat},${lon}`;
    locationsSeen.add(locationKey);
    let forecastRainfall;
    try {
      forecastRainfall = runtime2.runInNodeMode(fetchForecastRainfall, consensusMedianAggregation())(lat, lon, config.forecastDays).result();
      runtime2.log(`TokenId ${tokenId} [${locationKey}]: ` + `forecast = ${forecastRainfall.toFixed(1)}mm over ${config.forecastDays} days`);
    } catch (e) {
      forecastRainfall = Number(option.terms.strikeMM) + Number(option.terms.spreadMM);
      runtime2.log(`TokenId ${tokenId}: forecast fetch failed (${e.message}) — ` + `using conservative ${forecastRainfall}mm`);
    }
    const simulatedPayout = ethCall(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "simulatePayout", [tokenId, BigInt(Math.round(forecastRainfall))]);
    totalExpectedLoss += simulatedPayout;
  }
  runtime2.log(`Expected loss across ${activeOptions.length} options: ` + `${totalExpectedLoss.toString()} wei | ` + `Auto-settled: ${settledCount}`);
  const expectedLossBps = tvl > 0n ? Number(totalExpectedLoss * 10000n / tvl) : 0;
  runtime2.log(`Expected loss: ${expectedLossBps / 100}% of TVL | ` + `Alert threshold: ${config.utilizationAlertBps / 100}%`);
  let escrowBridgesTriggered = 0;
  for (const tokenId of activeOptions) {
    let option;
    try {
      option = ethCall(runtime2, evmClient, config.brumaAddress, BRUMA_ABI, "getOption", [tokenId]);
    } catch {
      continue;
    }
    if (Number(option.state.status) !== OptionStatus2.Settled)
      continue;
    const buyer = option.state.buyer;
    let isEscrow = false;
    try {
      isEscrow = ethCall(runtime2, evmClient, config.ccipEscrowFactoryAddress, FACTORY_ABI, "isRegisteredEscrow", [buyer]);
    } catch (e) {
      runtime2.log(`Factory check failed for ${buyer}: ${e.message}`);
      continue;
    }
    if (!isEscrow)
      continue;
    ...
    try {
      alreadyClaimed = ethCall(runtime2, evmClient, buyer, ESCROW_ABI, "claimed", [tokenId]);
    } catch (e) {
      runtime2.log(`Escrow claimed() check failed for tokenId ${tokenId}: ${e.message}`);
      continue;
    }
    if (alreadyClaimed) {
      runtime2.log(`TokenId ${tokenId}: escrow already claimed — skipping.`);
      continue;
    }
    try {
      runtime2.log(`TokenId ${tokenId}: triggering claimAndBridge on escrow ${buyer}...`);
      ethWrite(runtime2, evmClient, buyer, ESCROW_ABI, "claimAndBridge", [tokenId], config.gasLimit);
      escrowBridgesTriggered++;
      runtime2.log(`TokenId ${tokenId}: claimAndBridge submitted.`);
    } catch (e) {
      runtime2.log(`TokenId ${tokenId}: claimAndBridge failed — ${e.message}`);
    }
  }
  const criticalThresholdBreached = currentUtilBps >= config.utilizationAlertBps || expectedLossBps >= config.utilizationAlertBps;
  const criticalVolumeBreached = currentUtilBps >= config.criticalUtilizationBps || expectedLossBps >= config.criticalUtilizationBps;
  let reinsuranceTriggered = false;
  let actionLog = "";
  if (criticalVolumeBreached) {
    const newMaxBps = config.emergencyMaxUtilizationBps;
    const newTargetBps = Math.round(newMaxBps * 0.6);
    runtime2.log(`CRITICAL VOLUME BREACHED! ` + `Util=${currentUtilBps / 100}% / ExpectedLoss=${expectedLossBps / 100}%. ` + `Tightening vault to ${newMaxBps / 100}% max, ${newTargetBps / 100}% target...`);
    ...
    if (maxDrawableNow > 0n && totalExpectedLoss > 0n) {
      const drawAmount = totalExpectedLoss < maxDrawableNow ? totalExpectedLoss : maxDrawableNow;
      runtime2.log(`Drawing ${drawAmount.toString()} wei from ReinsurancePool ` + `(maxDrawableNow=${maxDrawableNow.toString()})...`);
      try {
        ethWrite(runtime2, evmClient, config.reinsurancePoolAddress, REINSURANCE_POOL_ABI, "receiveYield", [drawAmount], config.gasLimit);
        reinsuranceTriggered = true;
        runtime2.log(`ReinsurancePool draw submitted: ${drawAmount.toString()} wei`);
      } catch (e) {
        runtime2.log(`ReinsurancePool draw failed: ${e.message}`);
      }
    } else {
      runtime2.log(`ReinsurancePool draw skipped: ` + `maxDrawableNow=${maxDrawableNow.toString()} | ` + `totalExpectedLoss=${totalExpectedLoss.toString()}`);
    }
    actionLog = `CRITICAL ACTION TAKEN: vault tightened to ${newMaxBps / 100}% max. ` + `Reinsurance draw: ${reinsuranceTriggered ? "YES" : "SKIPPED (no capacity)"}. ` + `Expected loss: ${expectedLossBps / 100}% of TVL across ` + `${locationsSeen.size} unique locations. ` + `CCIP bridges triggered: ${escrowBridgesTriggered}. ` + `Options auto-settled: ${settledCount}.`;
    ...
    ethWrite(runtime2, evmClient, config.brumaVaultAddress, VAULT_ABI, "setUtilizationLimits", [BigInt(newMaxBps), BigInt(newTargetBps)], config.gasLimit);
    actionLog = `RISK ACTION TAKEN: vault tightened to ${newMaxBps / 100}% max. ` + `Expected loss: ${expectedLossBps / 100}% of TVL. ` + `CCIP bridges triggered: ${escrowBridgesTriggered}. ` + `Options auto-settled: ${settledCount}.`;
  } else {
    actionLog = `Vault healthy. Utilization: ${currentUtilBps / 100}%. ` + `Expected loss: ${expectedLossBps / 100}% of TVL. ` + `CCIP bridges triggered: ${escrowBridgesTriggered}. ` + `Options auto-settled: ${settledCount}. No vault action needed.`;
  }
  runtime2.log(actionLog);
  let riskSummary = "";
  try {
    const groqParams = {
      tvl: tvl.toString(),
...
var initWorkflow = (config) => {
  ...
  return [
    handler(settlementCron.trigger({ schedule: config.settlementSchedule }), onSettlementCron),
    handler(riskCron.trigger({ schedule: config.riskSchedule }), onRiskCron)
  ];
};
async function main() {
